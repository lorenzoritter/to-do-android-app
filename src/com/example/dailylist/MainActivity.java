...
public class MainActivity extends Activity {
    ...
    class SwipeTaskRow extends LinearLayout {
        ...
        void startTracking(MotionEvent event){
            ...
            beginDrag=()->{if(!isAttachedToWindow())return;if(activeEditTask==task)finishInlineEdit();dragging=true;performHapticFeedback(HapticFeedbackConstants.LONG_PRESS);setElevation(dp(8));setBackground(bg(PALE_BLUE,12));getParent().requestDisallowInterceptTouchEvent(true);};
        ...
        @Override public boolean onTouchEvent(MotionEvent event){
            switch(event.getActionMasked()){
                ...
                case MotionEvent.ACTION_MOVE:
                    float dx=event.getRawX()-downX,dy=event.getRawY()-downY;
                    if(dragging){setTranslationX(Math.max(-dp(52),Math.min(dp(52),dx)));setTranslationY(dy);setAlpha(.92f);return true;}
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if(dragging){float dragX=event.getRawX()-downX,dragY=event.getRawY()-downY;dragging=false;setElevation(0);setBackgroundColor(BACKGROUND);animate().translationX(0).translationY(0).alpha(1f).setDuration(140).withEndAction(()->finishTaskDrag(task,dragX,dragY)).start();return true;}
                ...
                case MotionEvent.ACTION_CANCEL:
                    cancelDragHold();getParent().requestDisallowInterceptTouchEvent(false);dragging=false;setElevation(0);resetPosition();swiping=false;return true;
            }
    ...
    void finishTaskDrag(Task task,float dragX,float dragY){
        if(Math.abs(dragX)<dp(30)&&Math.abs(dragY)<dp(31))return;if(activeEditTask==task)finishInlineEdit();ArrayList<Task> order=tasksForDate(task.date);int from=order.indexOf(task);if(from<0)return;
        ArrayList<Task> block=new ArrayList<>();block.add(task);if(!isSubtask(task))block.addAll(subtasks(task,false));order.removeAll(block);int insert=Math.max(0,Math.min(order.size(),from+Math.round(dragY/(float)dp(63))));long newParent=isSubtask(task)?task.parent:0;
        if(dragX>dp(40)&&block.size()==1){for(int i=Math.min(insert-1,order.size()-1);i>=0;i--){Task candidate=order.get(i);if(!isSubtask(candidate)){newParent=candidate.id;break;}}}else if(dragX<-dp(40))newParent=0;
        if(block.size()>1)newParent=0;task.parent=newParent;
        if(newParent!=0){int parentIndex=-1;for(int i=0;i<order.size();i++)if(order.get(i).id==newParent){parentIndex=i;break;}if(parentIndex<0){task.parent=0;newParent=0;}else{int groupEnd=parentIndex+1;while(groupEnd<order.size()&&order.get(groupEnd).parent==newParent)groupEnd++;insert=Math.max(parentIndex+1,Math.min(insert,groupEnd));}}
        if(newParent==0&&insert<order.size()&&isSubtask(order.get(insert))){long owner=order.get(insert).parent;int parentIndex=0;while(parentIndex<order.size()&&order.get(parentIndex).id!=owner)parentIndex++;int groupEnd=parentIndex+1;while(groupEnd<order.size()&&order.get(groupEnd).parent==owner)groupEnd++;insert=dragY<0?parentIndex:groupEnd;}
        order.addAll(Math.max(0,Math.min(insert,order.size())),block);applyDateOrder(task.date,order);save();render();
    }
